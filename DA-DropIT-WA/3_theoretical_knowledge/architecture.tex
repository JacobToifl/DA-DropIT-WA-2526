...
Dieses Unterkapitel gibt einen Überblick über Such- und Filterarchitekturen in Dokumentensystemen. Dabei werden lexikalische, semantische und hybride Suchverfahren sowie die Rolle von Filtern und Facetten beschrieben.
...
Dieses Unterkapitel gibt einen Überblick über Rollen- und Berechtigungssysteme als Grundlage der Zugriffskontrolle. Dabei werden Aufbau, Arten, Granularität und Stufen von Berechtigungen beschrieben.
...
Dieses Unterkapitel gibt einen Überblick über zentrale Anforderungen an skalierbare und plattformunabhängige Systemarchitekturen. Dabei werden Grundlagen der Skalierbarkeit sowie Aspekte der Portabilität beschrieben.

\subsubsection{Allgemeine Einordnung und Architekturarten}

Allgemein versteht man unter skalierbaren plattformunabhängigen Systemarchitekturen solche Architekturen, die ihre Funktionen über klar abgegrenzte Komponenten und Schnittstellen bereitstellen und sich an wachsende Lasten sowie unterschiedliche technische Umgebungen anpassen lassen. Welche konkrete Architektur gewählt wird, hängt vom jeweiligen Anwendungskontext, den Sicherheitsanforderungen und der gewünschten Wartbarkeit ab.

In der Literatur werden unter anderem folgende Architekturarten häufig beschrieben:
\begin{itemize}
    \item \textbf{Client-Server-Architektur:} Diese Architektur trennt die Benutzerinteraktion auf der Client-Seite von der Datenverarbeitung und Datenhaltung auf der Server-Seite. Dadurch können Oberflächen und Serverlogik weitgehend unabhängig weiterentwickelt werden, was sowohl die Portabilität als auch die Skalierbarkeit verbessert. \cite{fielding2000}
    \item \textbf{Microservices-Architektur:} Hier wird ein System in mehrere kleine, fachlich abgegrenzte Dienste unterteilt. Diese Dienste können unabhängig entwickelt, bereitgestellt und skaliert werden, wodurch sich die Architektur besonders für wachsende und sich häufig ändernde Systeme eignet. \cite{dragoni2017microservices}
    \item \textbf{Serviceorientierte Architektur (SOA):} Bei diesem Ansatz werden Funktionen als wiederverwendbare Services mit standardisierten Schnittstellen bereitgestellt. Dadurch lassen sich heterogene Systeme besser integrieren und organisatorisch getrennte Funktionen kontrolliert miteinander verbinden. \cite{papazoglou2006soa}
\end{itemize}
...
Für webbasierte Dokumentenablagen ergibt sich daraus ein konsistentes Architekturverständnis: Hohe Lasten und große Dokumentmengen lassen sich vor allem dann beherrschbar verarbeiten, wenn Verarbeitungsschritte modular getrennt, zustandsarme Dienste horizontal skalierbar, I/O-intensive Aufgaben asynchron ausführbar und häufig benötigte Daten gezielt gecacht oder partitioniert werden. Plattformunabhängigkeit wird dabei nicht allein durch die Wahl einer Laufzeit erreicht, sondern vor allem durch klar abgegrenzte Schnittstellen und eine geringe Kopplung an einzelne Infrastrukturkomponenten. Gleichzeitig darf Sicherheit nicht als nachträgliche Ergänzung behandelt werden, sondern muss über zentrale Identitätsverwaltung, klar definierte Autorisierung und konsistente Zugriffskontrollen als querschnittliche Eigenschaft der Gesamtarchitektur mitgeführt werden. \cite{studysmarter2024skalierbare,wikipedia2024plattformunabhaengigkeit,tsolkas2017rollen,msentra2025}
